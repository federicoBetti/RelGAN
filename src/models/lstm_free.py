...
class generator:
    def __init__(self, x_real, temperature, vocab_size, batch_size, seq_len, gen_emb_dim, mem_slots, head_size,
                 num_heads, hidden_dim, start_token, **kwargs):
        ...
        self.gen_mem = create_recurrent_unit(emb_dim=gen_emb_dim, hidden_dim=hidden_dim)
        self.g_output_unit = create_lstm_output_unit(hidden_dim, vocab_size)

        # Initial states
        h0 = tf.zeros([batch_size, hidden_dim])
        self.init_states = tf.stack([h0, h0])

        self.gen_recurrence()
        self.pretrain()

    def gen_recurrence(self):
        ...
        def _gen_recurrence(i, x_t, h_tm1, gen_o, gen_x, gen_x_onehot_adv):
            h_t = self.gen_mem(x_t, h_tm1)  # hidden_memory_tuple
            o_t = self.g_output_unit(h_t)  # batch x vocab, logits not probs
            gumbel_t = add_gumbel(o_t)
            next_token = tf.stop_gradient(tf.argmax(gumbel_t, axis=1, output_type=tf.int32))
            next_token_onehot = tf.one_hot(next_token, self.vocab_size, 1.0, 0.0)

            x_onehot_appr = tf.nn.softmax(
                tf.multiply(gumbel_t, self.temperature))  # one-hot-like, [batch_size x vocab_size]
            ...
            gen_o = gen_o.write(i,
                                tf.reduce_sum(tf.multiply(next_token_onehot, x_onehot_appr), 1))  # [batch_size], prob
        ...
        self.gen_o = tf.transpose(gen_o.stack(), perm=[1, 0])  # batch_size x seq_len
        self.gen_x = tf.transpose(gen_x.stack(), perm=[1, 0])  # batch_size x seq_len

        self.gen_x_onehot_adv = tf.transpose(gen_x_onehot_adv.stack(),
                                             perm=[1, 0, 2])  # batch_size x seq_len x vocab_size

    def pretrain(self):
        # ----------- pre-training for generator -----------------
        x_emb = tf.transpose(tf.nn.embedding_lookup(self.g_embeddings, self.x_real),
        ...
        self.pretrain_loss = -tf.reduce_sum(
            tf.one_hot(tf.to_int32(tf.reshape(self.x_real, [-1])), self.vocab_size, 1.0, 0.0) * tf.log(
                tf.clip_by_value(tf.reshape(self.g_predictions, [-1, self.vocab_size]), 1e-20, 1.0)
            )
        ) / (self.seq_len * self.batch_size)

    def pre_train_epoch(self, sess, g_pretrain_op, oracle_loader):
        supervised_g_losses = []
        oracle_loader.reset_pointer()

        for it in range(oracle_loader.num_batch):
            text_batch = oracle_loader.next_batch()
            _, g_loss = sess.run([g_pretrain_op, self.pretrain_loss], feed_dict={self.x_real: text_batch})
